        ...
        return (
          ...
          >
            <Link
              href={item.href}
              className={cn(
                "flex items-center gap-1 px-4 py-6 text-sm font-semibold uppercase tracking-wide transition-colors whitespace-nowrap",
                isActive
                  ? "text-brand-accent"
            ...
            {/* Invisible bridge — closes gap between link and dropdown */}
                      ...
                      <ul className="space-y-1">
                        {col.links.map((link) => (
                          <li key={link.label}>
                            <Link
                              href={link.href}
                              className="block py-1.5 text-sm text-brand-charcoal hover:text-brand-accent hover:pl-1 transition-all font-medium"
