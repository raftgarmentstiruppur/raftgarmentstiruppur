...
const FOLDERS = [
  ...
  // About & Company
  { label: "Infrastructure",           value: "Raft Garments/infrastructure",          section: "About" },
  { label: "Certifications",           value: "Raft Garments/certifications",          section: "About" },
  { label: "About / Team",             value: "Raft Garments/about",                  section: "About" },
  { label: "Sustainability",           value: "Raft Garments/sustainability",          section: "About" },
]
...
export default function MediaPage() {
  const [activeFolder, setActiveFolder] = useState(FOLDERS[0].value)
  ...
  const fetchImages = useCallback(async () => {
    setLoading(true)
    try {
      const res = await (await import("@/lib/api")).apiGet(`/media?folder=${encodeURIComponent(activeFolder)}`)
  ...
  async function deleteImage(publicId: string) {
    ...
    await (await import("@/lib/api")).apiDelete(`/media?public_id=${encodeURIComponent(publicId)}`)
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Folder tabs — grouped by section */}
      <div className="space-y-3">
        {["Products", "Homepage", "About"].map((section) => (
          <div key={section}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-brand-ash mb-1.5">{section}</p>
            <div className="flex flex-wrap gap-2">
              {FOLDERS.filter((f) => f.section === section).map((f) => (
                <button
                  key={f.value}
      ...
      <div className="border border-brand-border p-6 bg-white">
        <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-xs text-brand-ash">
            ...
            <a href="/admin/content" className="text-brand-accent underline">Admin → Content</a>{" "}
